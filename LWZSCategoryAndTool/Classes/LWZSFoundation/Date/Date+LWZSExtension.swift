...
extension Date {
    ...
    // MARK: 当月共有几天
    public func lwzs_countOfDaysInMonth() -> Int {
    ...
    // MARK: 当月第一天是星期几
    public func lwzs_firstWeekDay() -> Int {
    ...
    // MARK: 是否是这个月
    public func lwzs_isThisMonth() -> Bool {
        let calendar = NSCalendar.current
        let com = calendar.dateComponents([.year,.month,.day], from: self)
        let comNow = calendar.dateComponents([.year,.month,.day], from: Date())
    ...
    static public func lwzs_compare(date: Date, anotherDate: Date) -> Int {
    ...
    static public func lwzs_date(to date: Date, dateFormat: String = "yyyy-MM-dd") -> String {
    ...
    //  MARK: 日期字符串转date
    // dateFormat: yyyy-MM-dd HH:mm:ss / yyyy-MM-dd / MM-dd HH:mm ...
    static public func lwzs_date(to date: String, dateFormat: String = "yyyy-MM-dd") -> Date? {
    ...
    //  MARK: 时间比较
    static public func lwzs_dateDifference(date: Date, anotherDate: Date) -> Double {
        let interval = date.timeIntervalSince(anotherDate)
        return interval/86400
    }
    
    //  MARK: 时间转化成时间戳
    static public func lwzs_dateToTimeStamp(time: String, dateFormat: String = "yyyy-MM-dd") -> Int {
    ...
    //  MARK: 时间戳转化成时间
    static public func lwzs_date(from timeStamp: String, dateFormat: String = "yyyy-MM-dd") -> String {
    ...
    //  MARK: 获取当前时间戳
    static public func lwzs_timeStamp() -> Int {
        let date = Date()
        let timeInterval:Int = Int(date.timeIntervalSince1970)
        return timeInterval
    }
    
    static public func lwzs_date(date: String) -> String {
        let timeDate = self.lwzs_date(to: date)
        var result: String = ""
